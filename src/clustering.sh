...
per=$(awk '{sum+=$2} END{print sum}' .DAJIN_temp/clustering/tmp_"${suffix}" )
...
END{
    ...
    # -------------------------------
    # 同じ変異が2つ飛ばし以内で続いている場合は連続した変異とみなす
    # また、挿入塩基の場合は挿入塩基数を直接表記する
    # -------------------------------
    for(i in loc){
        if(loc[i+1] - loc[i] == 1 || loc[i+2] - loc[i] == 2 || loc[i+3] - loc[i] == 3) {num++}
        else if(ins[i]>0) {print cl[i], i, ins[i]""mut[i], loc[i]}
        else {print cl[i], i, num""mut[i], loc[i]; num=1}
}}' |
# -------------------------------
